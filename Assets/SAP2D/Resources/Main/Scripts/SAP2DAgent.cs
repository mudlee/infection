...
namespace SAP2D{
	...
	public class SAP2DAgent : MonoBehaviour {
		...
		IEnumerator FindPath(){ //path loop update
            if (Target != null)
            {
                if (isTargetWalkable())
                    ...
                    if (manager.grid.GetTileFromWorldPosition(transform.position).WorldPosition != manager.grid.GetTileFromWorldPosition(Target.position).WorldPosition)
                    {
                        path = manager.FindPath(transform.position, Target.position, Config);
                        pathIndex = 0;
                    }
            }
		...
		void Move(){ //object movement
			if (Target != null) {
				Vector3 targetVector = manager.grid.GetTileFromWorldPosition(Target.position).WorldPosition; //target tile position
				...
				if(CanSearch){
					if(transform.position != targetVector){
						if(path != null && path.Length > 0){
							...
							direction = currentTargetVector - transform.position; //direction of turn towards the current tile
							Rotate(direction);
		...
		bool isTargetWalkable(){
			return manager.grid.GetTileFromWorldPosition (Target.position).isWalkable;
		}
		
		void OnDrawGizmos(){
			if(ShowGraphic && Target!=null){
				Gizmos.color = color;
				...
				if(CanSearch){
					...
					Gizmos.DrawLine(transform.position, manager.grid.GetTileFromWorldPosition(Target.position).WorldPosition);
				}
